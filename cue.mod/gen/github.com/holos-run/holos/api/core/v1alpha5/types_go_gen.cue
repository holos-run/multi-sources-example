...
#Helm: {
	...
	// rendering the chart.  Values takes precedence over ValueFiles when both are
	// provided.
	values: #Values @go(Values)

	// ValueFiles represents helm hierarchial values in the order passed to helm
	// template -f flag.  Useful for migration from ApplicationSet that uses a
	// generator and the valueFiles field.  ValueFiles precede Values when both
	// are provided.
...
#ValueFile: {
	...
	// Values represents values for holos to marshal into the file name when
	// rendering the chart.
	values?: #Values @go(Values)
}
